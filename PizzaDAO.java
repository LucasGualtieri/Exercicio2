...
public class PizzaDAO extends DAO {
	...
	public Pizza get(char tamanho) {
		...
		try {
			...
			if (rs.next()) {
				pizza = new Pizza(rs.getString("sabor"), rs.getString("tamanho").charAt(0), rs.getDouble("preco"),
						rs.getString("borda"));
			}
			st.close();
		} catch (Exception e) {
	...
	private List<Pizza> get(String orderBy) {
		...
		try {
			...
			ResultSet rs = st.executeQuery(sql);
			while (rs.next()) {
				Pizza u = new Pizza(rs.getString("sabor"), rs.getString("tamanho").charAt(0), rs.getDouble("preco"),
						rs.getString("borda"));
				pizzas.add(u);
			}
			st.close();
